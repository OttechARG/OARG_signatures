...
let remitosInput = null; // NUEVO: input para remitos
    ...
    if (target && target.tagName === "LI") {
        ...
        if (recuperarBtn) {
            ...
            if (!recuperarBtn.hasAttribute("data-listener-added")) {
                ...
                async function recuperarDocumentoBase64ConReintentos(url, maxIntentos = 3) {
                ...
                class RemitoResolvers {
                    static async firmarDocumentoBase64(base64, url) {
...
async function llamarMutationSubirPdfBase64(base64) {
...
function mostrarPdfConOpciones(blob) {
...
let listaCompletaRemitos = []; // NUEVO
function showFieldsAssociatedWithPlanta() {
    if (!buscarCompaniaInput) {
        ...
        function mostrarRemitosEnLista(remitos) {
        ...
        async function cargarRemitos(cpyNombre, facilityCodigo) {
            try {
