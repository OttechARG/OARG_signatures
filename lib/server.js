...
app.get('/api/config/report', (req, res) => {
    res.json({ report });
});
// Report template endpoint with 3-level hierarchy fallback
app.get('/api/config/report-template', (req, res) => {
    try {
        const codsoc = req.query.codsoc;
...
app.get('/api/config/table-defaults', (req, res) => {
    try {
...
app.get('/api/config/table-customizations', (req, res) => {
    try {
...
app.post('/api/config/table-customizations', (req, res) => {
    try {
        const configPath = path.join(__dirname, '..', 'specific', 'table-customizations.json');
        const configDir = path.dirname(configPath);
        // Ensure directory exists
