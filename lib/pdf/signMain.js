...
function initializePuestoButtons() {
    const btnPuesto0 = document.getElementById("btnPuesto0");
    const btnPuesto1 = document.getElementById("btnPuesto1");
    if (btnPuesto0) {
...
function handlePuestoSelection(selectedPuesto) {
...
function updatePuestoButtonSelection(selectedPuesto) {
    const btnPuesto0 = document.getElementById("btnPuesto0");
    const btnPuesto1 = document.getElementById("btnPuesto1");
...
function createSaveButton() {
    ...
    const getRequirementsForPuesto = (puesto) => {
...
// Check if user IP matches server IP to show visual preferences
async function checkIpAccess() {
    try {
        const response = await fetch('/api/ip-match');
        const data = await response.json();
        const settingsDropdown = document.querySelector('.settings-dropdown');
        if (data.isMatch) {
            // Local user - show visual preferences button
            if (settingsDropdown) {
                settingsDropdown.style.display = 'block';
            }
            console.log('Local access granted - Visual preferences available');
        }
        else {
            // External user - hide visual preferences button
            if (settingsDropdown) {
                settingsDropdown.style.display = 'none';
            }
            console.log('External user - Visual preferences hidden');
        }
    }
    catch (error) {
        console.error('Error checking IP access:', error);
        // On error, hide the button for security
        const settingsDropdown = document.querySelector('.settings-dropdown');
        if (settingsDropdown) {
            settingsDropdown.style.display = 'none';
        }
...
window.refreshCurrentTable = (page = 1, pageSize) => {
