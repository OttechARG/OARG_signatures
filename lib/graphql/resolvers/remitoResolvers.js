...
export const remitoResolvers = {
    ...
    Mutation: {
        ...
        async guardarJsonArchive(_, { key, jsonData }) {
            if (!key)
                throw new Error("Falta key");
            if (!jsonData)
                throw new Error("Falta jsonData");
            const data = readSignedPdfs();
