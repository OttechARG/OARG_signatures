...
export class TableHandler {
    ...
    async goToPage(page) {
        ...
        try {
            const remitosHandler = window.remitosHandler;
            if (remitosHandler) {
                const result = await remitosHandler.fetchRemitos(this.currentParams.company, this.currentParams.facility, this.currentParams.fechaDesde, page, pageSize, currentFilter);
                await this.renderTable(result.remitos, result.pagination);
            }
        }
        catch (error) {
