    ...
    async renderTable(remitos, pagination, dynamicColumns) {
        ...
        if (dynamicColumns && dynamicColumns.length > 0) {
            ...
            if (window.clientTableConfigManager) {
                window.clientTableConfigManager.updateConfigWithSqlColumns(dynamicColumns);
                // Get filtered columns (only visible ones)
                this.columns = window.clientTableConfigManager.getVisibleColumnsFilteredBySql(dynamicColumns);
            }
            else {
