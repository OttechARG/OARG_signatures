...
import { getReportTemplate } from "../core/ReportConfig.js";
export class TableHandler {
    ...
    async waitForConfigManagerAndApply(dynamicColumns) {
        const maxAttempts = 30; // 3 seconds max wait (increased for config loading)
        let attempts = 0;
        return new Promise((resolve) => {
            const checkAndApply = () => {
                attempts++;
                const manager = window.clientTableConfigManager;
    ...
    async initializeWithSavedConfig() {
    ...
    async renderTable(remitos, pagination, dynamicColumns) {
