...
function consultarCompanias() {
  fetch(`/api/companias`)
    .then(res => res.json())
    .then((data: { CPY_0: string; CPYNAM_0: string }[]) => {
...
function mostrarCompaniasEnLista(companias: { CPY_0: string; CPYNAM_0: string }[]) {
  const listaCompanias = document.getElementById("companiasList") as HTMLUListElement | null;
  if (!listaCompanias) return;
    ...
    if (buscarCompaniaInput) {
      ...
      buscarCompaniaInput.addEventListener("blur", () => {
        setTimeout(() => {
          ...
          if (listaCompanias) listaCompanias.style.display = "none";
        }, 150);
      });
    }
